...
int isLineFit(double lineLength, const shape_t *objp);
...
int main()
{
    ...
    double line_Length;
    printf("\nEnter length of the line segment: ");
    scanf("%lf",&line_Length);

    printf("\nThe given line with length %.2lf can fit the following shapes:\n",line_Length);
    for (int i = 0; i < numOfShapes; i++)
        if (isLineFit(line_Length,&shapes[i]))
            printShape(&shapes[i]);
...
int isLineFit(double lineLength, const shape_t *objp)
{
    if(objp->type == 0)
    {
        if(lineLength <= objp->shape.cube.side*sqrt(3)){
            return 1;
        }
        return 0;
    }
    ...
    {
        if(lineLength <= sqrt(2*pow(objp->shape.square_prism.base_side_length, 2)+pow(objp->shape.square_prism.height, 2))){
            return 1;
        }
        return 0;
    }
    ...
    {
        if(lineLength <= 2*objp->shape.sphere.radius){
            return 1;
        }
        return 0;
    }
